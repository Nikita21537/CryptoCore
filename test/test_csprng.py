...
class TestCSPRNG(unittest.TestCase):
    ...
    def test_is_key_weak(self):
        ...
        strong_keys = [
            ...
            # Убрали последовательные байты - они слабые!
            generate_aes_key(),  # Случайный ключ не должен быть слабым
        ]
        ...
        weak_keys = [
            ...
            bytes([i for i in range(16)]),  # Simple increasing sequence (00 01 02 ...)
            bytes([0xAA] * 16),  # Repeated byte
            bytes([0xAB, 0xCD] * 8),  # Repeated pattern
            bytes.fromhex('000102030405060708090a0b0c0d0e0f'),  # Sequential bytes
        ]
    ...
    def test_cli_key_generation(self):
        """Test CLI key generation during encryption."""
        ...
        try:
            ...
            # ДЕКОДИРУЕМ вывод из байтов в строку
            output = result.stdout.decode('utf-8', errors='ignore') + result.stderr.decode('utf-8', errors='ignore')

            self.assertEqual(result.returncode, 0,
                             f"Encryption failed: {output}")
            ...
            import re
            key_match = re.search(r'Generated key: ([0-9a-f]{32})', output)
            self.assertIsNotNone(key_match, f"Could not find generated key in output:\n{output}")
            ...
            output2 = result.stdout.decode('utf-8', errors='ignore') + result.stderr.decode('utf-8', errors='ignore')
            self.assertEqual(result.returncode, 0,
                             f"Decryption failed: {output2}")
